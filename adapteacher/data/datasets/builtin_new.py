...
def register_all_clipart(root):
    ...
    # first name of the split is for identifying the name dataset in config file
    SPLITS = [
...
def register_voc2water(root):
    # root = "manifold://mobile_vision_dataset/tree/yujheli/dataset"
    SPLITS = [
        ("VOC2water_train", "voc_cycle_water/VOC2012", "train"),
        ("VOC2water_test", "voc_cycle_water/VOC2012", "test"),
    ]
    for name, dirname, split in SPLITS:
        year = 2012
        # register_pascal_voc(name, os.path.join(root, dirname), split, year, class_names=["person", "dog","bicycle", "bird", "car", "cat"])
...
def register_voc2clipart(root):
    # root = "manifold://mobile_vision_dataset/tree/yujheli/dataset"
    SPLITS = [
        ("VOC2clipart_train", "voc_cycle_clipart/VOC2012", "train"),
        ("VOC2clipart_test", "voc_cycle_clipart/VOC2012", "test"),
    ]
    for name, dirname, split in SPLITS:
        year = 2012
        # register_pascal_voc(name, os.path.join(root, dirname), split, year, class_names=["person", "dog","bicycle", "bird", "car", "cat"])
        register_pascal_voc(name, os.path.join(root, dirname), split, year)
        MetadataCatalog.get(name).evaluator_type = "pascal_voc_clipart"
        # MetadataCatalog.get(name).thing_classes = ["person", "dog","bike", "bird", "car", "cat"]
        # MetadataCatalog.get(name).thing_classes = ["person", "dog","bicycle", "bird", "car", "cat"]
        # MetadataCatalog.get(name).evaluator_type = "coco"

# TODO: cyclgan images are all in voc format
def register_cityscape(root):
    SPLITS = [
        ("city2foggy_trainval", "/city_cycle_foggy/", "trainval")
        ("city2foggy_test", "/city_cycle_foggy/", "test")]

    for name, dirname, split in SPLITS:
        year = 2012
        # register_pascal_voc(name, os.path.join(root, dirname), split, year, class_names=["person", "dog","bicycle", "bird", "car", "cat"])
        register_pascal_voc(name, os.path.join(root, dirname), split, year)
        MetadataCatalog.get(name).evaluator_type = "city2foggy"


register_all_cityscapes_foggy(_root)
register_all_clipart(_root)
register_all_water(_root)
register_voc2water(_root)
register_voc2clipart(_root)
